...
void MPC::init(ros::NodeHandle &nh)
{
    ...
    bspline_sub_ = nh.subscribe("planning/bspline", 10, &MPC::bsplineCallback,this);
    ...
    if (!in_test)
    {
        ...
        if (t_cur <= traj_duration_ && t_cur >= 0.0)
        {
            ...
            for (int i=0; i<T; i++)
            {
                ...
                // if (t_temp < downvt)
                // {
                //     temp = traj_[0].evaluateDeBoorT(t_temp);
                //     xref(2, i) = 1.5;
                // }
        ...
        else if (t_cur > traj_duration_)
        {
            ...
            if((final_pos-now_pos).norm()>1.0)
            {
                cmd.linear.x = traj_[1].evaluateDeBoorT(traj_duration_-dt).norm();
                Eigen::Vector3d dir = traj_[0].evaluateDeBoorT(t_cur + 20*dt) - now_pos;
                yaw_temp = 2*  atan2(dir(1), dir(0)) ;
            }
            else if((final_pos - now_pos).dot(traj_[1].evaluateDeBoorT(traj_duration_- 1*dt))>0.05)//还在朝者这个方向走
            {
                cmd.linear.x = traj_[1].evaluateDeBoorT(traj_duration_-dt).norm();
...
void MPC::predictMotion(void)//why to predict
{
    xbar[0] = now_state;
