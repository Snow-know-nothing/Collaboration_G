...
namespace ego_planner
{
  ...
  void BsplineOptimizer::setParam(ros::NodeHandle &nh)
  {
    ...
    nh.param("optimization/lambda_limitz", lambda5_, 100.0);
  ...
  std::vector<ControlPoints> BsplineOptimizer::distinctiveTrajs(vector<std::pair<int, int>> segments)
  {
    ...
    for (int i = 0; i < seg_upbound; i++)
    {
      ...
      exit_multi_loop2:;
        ...
        for (int j = occ_start_id; j <= occ_end_id; j++)
        {
          ...
          {
            base_pt_reverse = occ_end_pt;    //反向牵引点
          }
          else
          {
          ...
          if (grid_map_->getInflateOccupancy(base_pt_reverse)) // Search outward.如果反向牵引点在膨胀栅格地图的障碍物内，
          {
    ...
    {
      // 2.1 Calculate the selection table.这是什么
      int digit_id = 0;
      selection[digit_id]++;
...
//最后将障碍物控制点对外的控制点保持原来的控制点
        ...
        //于障碍物控制点对内的控制点，
        //只有一段控制点对内的控制点选择正向牵引点和牵引方向，其余段都选择反相。
    ...
    for (int i = order_; i <= i_end; ++i)
    {
      ...
      for (double a = 1.0; a > 0.0; a -= step_size)
      {
        ...
        if (occ && !last_occ)
        {
          ...
          {
            in_id = i - 1;
            flag_got_start = true;//进障碍物标志位置
          }
          same_occ_state_times = 0;  //连续有/无障碍物计数器置
          flag_got_end_maybe = false; // terminate in advance
        }
        else if (!occ && last_occ)
        {
          out_id = i;
          flag_got_end_maybe = true;  //可能出障碍物标志位置
        ...
        if (flag_got_end_maybe && (same_occ_state_times > ENOUGH_INTERVAL || (i == (int)init_points.cols() - order_)))//为最后一段了
        {
      ...
      if (segment_ids[i].second - segment_ids[i].first == 1)
      {
        ...
        while (Astar_id >= 0 && Astar_id < (int)a_star_pathes[i].size())
        {
          ...
          if (val * last_val <= 0 && (abs(val) > 0 || abs(last_val) > 0)) // val = last_val = 0.0 is not allowed
          {
            ...
            {
              //如果牵引点与中点的距离大于1cm， 该控制点对应的cps_.flag_temp置为true，
  ...
  void BsplineOptimizer::calcSwarmCost(const Eigen::MatrixXd &q, double &cost, Eigen::MatrixXd &gradient)
  {
    ...
    {
      double glb_time = t_now + ((double)(order_ - 1) / 2 + (i - order_ + 1)) * bspline_interval_;//这个
  ...
  void BsplineOptimizer::calcLimitzvelCost(const Eigen::MatrixXd &q, double &cost,
...
//#define USE_SECOND_CLEARENCE_CHECK
  ...
  void BsplineOptimizer::combineCostRebound(const double *x, double *grad, double &f_combine, const int n)
  {
    ...
    double f_smoothness, f_distance, f_feasibility /*, f_mov_objs*/, f_swarm, f_terminal, f_limitz;
    ...
    Eigen::MatrixXd g_terminal = Eigen::MatrixXd::Zero(3, cps_.size);
     Eigen::MatrixXd g_limitz = Eigen::MatrixXd::Zero(3, cps_.size);
    ...
    calcLimitzvelCost(cps_.points, f_limitz, g_limitz);

    f_combine = lambda1_ * f_smoothness + new_lambda2_ * f_distance + lambda3_ * f_feasibility + new_lambda2_ * f_swarm + lambda2_ * f_terminal + lambda5_*f_limitz;
    //f_combine = lambda1_ * f_smoothness + new_lambda2_ * f_distance + lambda3_ * f_feasibility + new_lambda2_ * f_mov_objs;
    //printf("origin %f %f %f %f\n", f_smoothness, f_distance, f_feasibility, f_combine);

    Eigen::MatrixXd grad_3D = lambda1_ * g_smoothness + new_lambda2_ * g_distance + lambda3_ * g_feasibility + new_lambda2_ * g_swarm + lambda2_ * g_terminal + lambda5_*g_limitz;
