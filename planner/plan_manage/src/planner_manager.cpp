...
//使用L-BFGS优化控制点，得到最优轨迹控制点。
  ...
  //car5
    ...
    do
    {
      ...
      if (flag_first_call || flag_polyInit || flag_force_polynomial /*|| ( start_pt - local_target_pt ).norm() < 1.0*/) // Initial path generated from a min-snap traj by order.
      {
        ...
        //根据路程的需要判断时间计算是否考虑到加到速度最大匀速的时间段
        ...
        {
          Eigen::Vector3d horizen_dir = ((start_pt - local_target_pt).cross(Eigen::Vector3d(0, 0, 1))).normalized();  //x,y平面的水平单位向量
          Eigen::Vector3d vertical_dir = ((start_pt - local_target_pt).cross(horizen_dir)).normalized();//垂直单位向量
      ...
      else // Initial path generated from previous trajectory.
      {
        ...
        vector<double> pseudo_arc_length;//分段距离累加赋值
  ...
  //障碍物两边紧挨的控制点对，再向两边扩展后得到调整后的障碍物控制点） 
    ...
    if (pp_.use_distinctive_trajs)
    {
      ...
      for (int i = trajs.size() - 1; i >= 0; i--)
      {
        ...
        //final_cost和优化后的控制点ctrl_pts_temp是要得到的
        {
  ...
  bool EGOPlannerManager::checkCollision(int drone_id)//多飞机clli
  {
...
//只给了起点和终点
                                         ...
                                         const Eigen::Vector3d &end_pos, const Eigen::Vector3d &end_vel, const Eigen::Vector3d &end_acc)
  {
    ...
    vector<Eigen::Vector3d> points;
    points.push_back(start_pos);
    points.push_back(end_pos); //car4

    // insert intermediate points if too far
    vector<Eigen::Vector3d> inter_points;
    const double dist_thresh = 4.0;

    for (size_t i = 0; i < points.size() - 1; ++i)   //按照阈值插值
    {
      inter_points.push_back(points.at(i));
      double dist = (points.at(i + 1) - points.at(i)).norm();
    ...
    int pt_num = inter_points.size();
    Eigen::MatrixXd pos(3, pt_num);
    for (int i = 0; i < pt_num; ++i)    //将插值后的路径点转换成矩阵的列形式
      pos.col(i) = inter_points[i];
  ...
  void EGOPlannerManager::updateTrajInfo(const UniformBspline &position_traj, const ros::Time time_now)
  {
    ...
    local_data_.traj_id_ += 1;   //轨迹的id的作用
  }
